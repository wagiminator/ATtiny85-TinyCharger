// ===================================================================================
// Project:   TinyCharger - Li-Ion Battery Charger based on ATtiny25/45/85
// Version:   v1.1
// Year:      2021
...
// Description:
// ------------
// TinyCharger is a simple single cell Li-Ion battery charger with selectable
// charging current and OLED display for charging monitoring. The TP4056 is used
// for charging. The charging current is programmed via a resistor, the value of
// which can be set by the ATtiny via two MOSFETs and changed by pressing the
// SET button. An INA219 continuously measures the charging current and voltage
// and transmits the values to the ATtiny via I2C. From this, the ATtiny calculates
// the state of charge and capacity and displays the values on the OLED screen.
//
// References:
// -----------
...
// PROG1 ------- ADC3 PB3  2|    |7  PB2 ADC1 -------- OLED/INA SCK
// PROG2 ------- ADC2 PB4  3|    |6  PB1 AIN1 OC0B --- SET BUTTON
...
// -----------------------
// Connect the device via the Micro-USB or USB-C port to a 5V power supply that
// can provide sufficient current. Use the SET button to select the maximum
// charging current. Note that it can overshoot by a few milliamperes! Connect
// the Li-Ion battery to one of the battery connectors. Pay attention to the
// correct polarity! The battery is charged immediately. The SET button is locked
// during the charging process to prevent the charging current from being changed
// accidentally. The charging process stops automatically when the battery is
// fully charged. The total charging time and the charged capacity remain
// displayed as long as the device is supplied with power. These values can be
...
// Pin definitions
#define PIN_SDA       PB0                   // I2C serial data pin  (OLED/INA219)
#define PIN_SCL       PB2                   // I2C serial clock pin (OLED/INA219)
#define PIN_SET       PB1                   // pin connected to SET button
#define PIN_PROG1     PB3                   // charging current prog pin 1
#define PIN_PROG2     PB4                   // charging current prog pin 2
...
const uint16_t PROG_CURRENT[] PROGMEM = {100, 350, 750, 1000};
const uint16_t BAT_LEVEL[]    PROGMEM = {3000, 3600, 4000, 4100, 65535};
...
const uint16_t DIVIDER[] PROGMEM = {1000, 100, 10, 1};
...
int main(void) {
  ...
  uint8_t   isCharging;                         // charging state  (0: not charging)
  ...
  // Setup
  pinPullup(PIN_SET);                           // pullup on button pin
  pinOutput(PIN_PROG1);                         // prog pins as output
  pinOutput(PIN_PROG2);                         // prog pins as output
  ...
  while(1) {  
    ...
    while(voltage > pgm_read_word(&BAT_LEVEL[batState])) batState++;
    ...
    if(pinRead(PIN_SET)) lastbutton = 0;
    else if(!lastbutton) {
      if(!isCharging) {
        if(++progState > 3) progState = 0;
        (progState & 0x01) ? pinHigh(PIN_PROG1) : pinLow(PIN_PROG1);
        (progState & 0x02) ? pinHigh(PIN_PROG2) : pinLow(PIN_PROG2);
      }
      lastbutton  = 1;
    }
    ...
    OLED_printDec4(pgm_read_word(&PROG_CURRENT[progState]));
    OLED_printString(mA);
